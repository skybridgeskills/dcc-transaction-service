...
export const getVerifyVPR = (exchange: App.ExchangeDetailVerify) => {
  ...
  // Guard against undefined vprContext - use default if missing
  const safeVprContext = vprContext || [CONTEXT_URL_V1]
  const serviceEndpoint = `${exchange.variables.exchangeHost}/workflows/${exchange.workflowId}/exchanges/${exchange.exchangeId}`

  const specificContexts = safeVprContext.filter(
  ...
  const credentialQueries = safeVprContext.some((c) =>
    [CONTEXT_URL_V1, CONTEXT_URL_V2].includes(c)
  )
    ? [
        getCredentialQuery({
          vprContext: safeVprContext,
          vprCredentialType,
          trustedIssuers,
          vprClaims
        })
