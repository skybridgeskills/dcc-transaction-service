...
export class FakeIssuerService implements IssuerService {
	...
	private async initializeKey(issuerDid?: string, issuerKeySeed?: Uint8Array): Promise<void> {
		...
		if (issuerDid) {
			...
			} else if (this.defaultKey.id && String(this.defaultKey.id).startsWith('did:key:')) {
				// If key.id is already a did:key, use it
				controller = String(this.defaultKey.id).split('#')[0];
			} else {
				throw new Error('Failed to generate did:key from public key. Key document: ' + JSON.stringify(keyDoc, null, 2));
			}
		}
