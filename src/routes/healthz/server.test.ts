...
describe('GET /healthz', function () {
  ...
  test('returns error status if HTTPException thrown', async function () {
    // Create a mock keyValueStore that throws HTTPException
    // Note: HTTPException status is preserved by the endpoint
    const mockKeyValueStore: KeyValueStoreService = {
      get: vi
        .fn()
        .mockRejectedValue(
          new HTTPException(500, { message: 'Failed to save exchange.' })
        ),
